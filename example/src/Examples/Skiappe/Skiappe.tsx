import {Canvas, Text, useFont, Fill, Skia, SkTypeface, vec, Glyphs, useClockValue, useComputedValue, Rect, useSpring, Selector, bottomRight, LinearGradient, topLeft, Points, Vertices, Group} from "@shopify/react-native-skia";
...
export const Skiappe = ({assets}: Props) => {
  ...
  const text = useMemo(() => glyphs
    ...
    let counterWidth2 = 0;
    const text2 = useMemo(() => glyphs
    .map((id, i) => {
      counterWidth2 += i > 0 ? widths[i-1] : 0

      return ({ id, pos: vec(counterWidth2, 160 - (0/*lineLetterByLetter.l[i].o * 100*/)) })
    }), [glyphs, widths]);


    let counterWidth3 = 0;
    const text3 = useMemo(() => glyphs
    .map((id, i) => {
    ...
    const opacities = useComputedValue(() => {
        ...
        return glyphs.map((id, i) => {
            ...
            if (progress <= letterStart ) {
                ...
                const letterProgress = ((progress * 100 - letterStart *100) / duration) * 100
  ...
  return (
    <>
      ...
      <Group transform={[{ translateY: 300},{ translateX: 300 }]}>
        ...
        <Group transform={[{ rotate: -0.95}, {translateY: 25}, {translateX: -150}]}>
        ...
        <Group transform={[{ rotate: -0.95}, {translateY: 250}, {translateX: -150}]}>
        {text3.map((gliph, i) => <Glyphs
          key={i}
          font={font}
          glyphs={[gliph]}
        ...
        <Group transform={[{ rotate: -0.95}, {translateY: 175}, {translateX: -150}]}>
        {text3.map((gliph, i) => <Glyphs
          key={i}
          font={font}
          glyphs={[gliph]}
        ...
        <Group transform={[{ rotate: -0.95}, {translateY: 100}, {translateX: -150}]}>
        {text3.map((gliph, i) => <Glyphs
          key={i}
          font={font}
          glyphs={[gliph]}
...
{/*         
        ...
        {/* <Glyphs
          ...
          opacity={opacity}
        /> */}
        <Vertices transform={[{scale: 2}, {translateY: -200}, {translateX: -50}]} vertices={vertices} colors={colors} />
        {/* <Text
          x={0}
          y={fontSize}
